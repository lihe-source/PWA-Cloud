...
- 檔案、備註與相片列表改為固定表頭、垂直與水平可滾動的緊湊表格。
- 手機版取消一筆資料一張大型卡片的排版，可在同一畫面查看更多項目。
- 相片表格提供縮圖、檔名、上傳者、上傳時間、檔案大小、預覽及複製操作。
- Safari／iOS 複製圖片改為在使用者點擊授權仍有效時立即呼叫 Clipboard API，並以 Promise 完成圖片讀取與 PNG 轉換。
- 圖片內容 API 支援 `?inline=1`，降低行動瀏覽器讀取附件時發生 `Load failed` 的機率。
- 相同錯誤通知會自動去重，避免短時間內顯示多個重複提示。
