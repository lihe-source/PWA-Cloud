...
> V2.9.0 完成 App icon、favicon 與 iOS 啟動畫面的一致化，所有品牌資產均沿用同一款扁平上傳／儲存圖示。

DriveDock 是一套可安裝在 iPhone、Android 與桌面瀏覽器的 PWA 檔案櫃。V2.9.0 採用 **GitHub Pages 純前端架構**：使用 Google Identity Services 取得短效 Access Token，再由瀏覽器直接呼叫 Google Drive REST API。本版主要更新為全新的扁平化上傳／儲存圖示，並保留備註刪除、扁平緊湊介面、Blob 下載、相片卡片、iPhone 圖片複製與登入狀態恢復。
...
V2.9.0 使用完整 Google Drive scope，才能直接存取使用者貼入的既有 Folder ID、共享資料夾及 V1.3.0 已建立的資料。Google OAuth 可能顯示較廣泛的 Drive 授權說明。
